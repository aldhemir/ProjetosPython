...
class instagramBot:
    def __init__(self, username, password, verificacaoCodigo):
        ...
        # ATENÇÃO: Verifique se este caminho abaixo reflete a pasta real no seu computador
        # Se você usa Windows, descomente a linha abaixo e ajuste o caminho:
        #self.driver = webdriver.Firefox(executable_path=r"C:\geckodriver.exe") 
        
        # Caminho ajustado para o usuário Aldhemir (macOS/Linux)
    ...
    def login(self):
        ...
        # Tratamento para verificar se o campo de 2FA realmente aparece, para evitar erro se o insta não pedir
        try:
            if self.verificacaoCodigo:
    ...
    def seguindo(self, perfil_instagram):
        ...
        for i in range(1, int(total_seguindo)):
            ...
            # Verificação simples para evitar erro de índice
            if len(seguindo) > 1:
                seguindo[1].click() #clica no botão para seguir
...
#Janela
janela = sg.Window('BOT INSTAGRAM - ALDHEMIR MACEDO', layout)
...
while True:
    ...
    if eventos == 'Logar':
        if valores['username'] == '' or valores['password'] == '':
            ...
            # Instanciando o bot com o novo nome da variável
