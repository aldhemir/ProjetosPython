...
class instagramBot:
    def __init__(self, username, password, verificacaoCodigo):
        ...
        # ATENÇÃO: Preencher o caminho do geckodriver abaixo
    ...
    def login(self):
        ...
        time.sleep(3)
        # Seletores podem precisar de atualização dependendo da versão do Instagram
        ...
        # O código abaixo assume que a verificação de 2 fatores SEMPRE aparece.
        # Se não aparecer, vai dar erro. Pode ser necessário um try/except aqui.
        try:
            ...
            print("Verificação de dois fatores não solicitada ou elemento não encontrado.")
    ...
    def comentar_no_sorteio(self):
        driver = self.driver
        # ATENÇÃO: Link do sorteio hardcoded abaixo
        ...
        try:
            ...
            print("Botão de curtir não encontrado ou já curtido.")
            
        time.sleep(2)
        try:
            contador = 0
            # ATENÇÃO: Preencher caminho do Excel abaixo
            self.lista_seguidores = pd.read_excel(r" ") 
            
            tamanho = len(self.lista_seguidores['username']) 
            for perfil_seguidores in range(0, tamanho, 2): 
                time.sleep(5)      
...
while True:
    ...
    if eventos == 'Logar':
        if valores['username'] == '' or valores['password'] == '' or valores['vefificacaoCodigo'] == '':
            ...
            # Instância renomeada para aldhemirBot
