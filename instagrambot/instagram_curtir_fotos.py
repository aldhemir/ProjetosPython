...
class instagramBot:
    def __init__(self, username, password, hashtag_procurar, verificacaoCodigo):
        ...
        # ATENÇÃO: Verifique se este caminho abaixo reflete o local real no seu computador
    ...
    def curtir_fotos(self, hashtag):
        ...
        for imagem_href in imagem_hrefs:
            ...
            try:
                time.sleep(5)
                # OBS: O Instagram muda as classes (como '_aamw') frequentemente. 
                # Se parar de funcionar, inspecione o botão de like novamente.
...
while True:
    ...
    if eventos == 'Logar':
        if valores['username'] == '' or valores['password'] == '' or valores['hashtag'] == '' or valores['vefificacaoCodigo'] == '':
            ...
            aldhemirBot = instagramBot(valores['username'], valores['password'], valores['hashtag'], valores['vefificacaoCodigo'] )
            aldhemirBot.login()

#verificacaoCodigo = input("Digite o código de verificação: ")
#aldhemirBot = instagramBot("SEU_USERNAME", "SEU_PASSWORD", verificacaoCodigo )
#aldhemirBot.login()